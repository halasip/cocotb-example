...
##### for remote debugging, with GUI attach command place in cocotb test function, not in main
```py
    import debugpy
...
```py
    await cocotb.triggers.ClockCycles(dut.clk, nbrClocks, rising=True)
```
...
##### Python basic assertion. If values are different, raises an error.
```py
...
##### Function with time delay elements, use async keyword and pass dut with other parameters
```py
...
##### cocotb test must be preceded by decorator. multiple tests may be included in the same python module (file)
```py
    @cocotb.test()
...
##### Cocotb test with only a wait command. Useful to confirm that GHDL compiles all the modules and can start without errors
```py
